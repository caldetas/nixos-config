...
with lib;
{
  ...
  config = mkIf (config.backup.enable) {
    ...
    systemd.services.borgmatic-prepare-env = {
      ...
      serviceConfig = {
        Type = "oneshot";
        ExecStart = pkgs.writeShellScript "borgmatic-prepare" ''
          ...
          if ! ${pkgs.gnugrep}/bin/grep -q "$host1" /root/.ssh/known\_hosts; then
            ${pkgs.openssh}/bin/ssh-keyscan -p 23 "$host1" >> /root/.ssh/known_hosts
            ${pkgs.openssh}/bin/ssh-keyscan -p 23 "$host2" >> /root/.ssh/known_hosts
          fi
        '';
